...
class SearchService{
    static searchUser = async (query) => {
        ...
        const users = user.find({}).select('firstName lastName email'); // Fetch only necessary fields
        const words = users.map(user => user.firstName).concat(users.map(user => user.lastName)).concat(users.map(user => user.email))
        ...
        client.set(query, JSON.stringify(foundUser), 'EX', 3600); // cache result for 1 hour
